...
fn process_event(_dev: &Device, ev: &InputEvent, hotkey: bool) {
    ...
    if hotkey{
        if ev.event_code == BRIGHT && ev.value == -1 {
            ...
            Command::new("brightnessctl").args(&["-n1","s","2%-"]).output().expect("Failed to execute brightnessctl");
            //Command::new("brightnessctl").arg("-O").output().expect("Failed to execute brightnessctl");
        }
            ...
            Command::new("brightnessctl").args(&["-n1","s","1%-"]).output().expect("Failed to execute brightnessctl");
            //Command::new("brightnessctl").arg("-O").output().expect("Failed to execute brightnessctl");
        }
